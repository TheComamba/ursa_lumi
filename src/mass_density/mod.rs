...
pub(super) fn rho0(population: Population, age: Time) -> f64 {
    match population {
        Population::ThinDisc(Subpopulation::Alive) => {
            if age < Time::new::<gigayear>(0.15) {
                4.0e-3
            } else if age < Time::new::<gigayear>(1.) {
                7.9e-3
            } else if age < Time::new::<gigayear>(2.) {
                6.2e-3
            } else if age < Time::new::<gigayear>(3.) {
                4.0e-3
            } else if age < Time::new::<gigayear>(5.) {
                4.0e-3
            } else if age < Time::new::<gigayear>(7.) {
                4.9e-3
            } else {
                6.6e-3
            }
...
mod tests {
    ...
    fn rho0_is_correct() {
        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(0.1),
        );
        assert_ratio!(4.0e-3, rho, ACC);

        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(0.5),
        );
        assert_ratio!(7.9e-3, rho, ACC);

        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(1.5),
        );
        assert_ratio!(6.2e-3, rho, ACC);

        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(2.5),
        );
        assert_ratio!(4.0e-3, rho, ACC);

        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(3.5),
        );
        assert_ratio!(4.0e-3, rho, ACC);

        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(5.5),
        );
        assert_ratio!(4.9e-3, rho, ACC);

        let rho = rho0(
            Population::ThinDisc(Subpopulation::Alive),
            Time::new::<gigayear>(7.5),
        );
        ...
        let rho = rho0(Population::Spheroid, Time::new::<gigayear>(0.));
        assert_ratio!(9.32e-6, rho, ACC);

        let rho = rho0(Population::Bulge, Time::new::<gigayear>(0.));
    ...
    fn epsilon_is_correct() {
        ...
        let eps = epsilon(Population::Spheroid, Time::new::<gigayear>(0.));
        assert_ratio!(0.76, eps, ACC);

        let eps = epsilon(Population::Bulge, Time::new::<gigayear>(0.));
        assert_diff!(0., eps, ACC);
    }
}
